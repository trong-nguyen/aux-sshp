...
def simulate(initial_weight, data_file):
    '''
    descriptions: a standard read-check-run-display flow of simulators
        if users only want the raw input-to-output function, use the simulate_weight instead

    params:
        - initial_weight: the weight of the fish on day 1 of recorded temperature data
        - data_file: path to the temperature data_file
            + data_file format should be .csv, with labels, temperatures in 3rd column
...
def simulate_weight(w0, temperatures):
    '''
    descriptions: the main computation of final weight given inputs

    params:
        w0: initial weight
        temperatures: an array contains temperature on a range of continuous day

    return:
        final weight after last day of temperature recording
    '''
...
def read_data(data_file):
    '''
    descriptions: reading temperature data from a specified csv file

    return:
        - an array of recorded temperatures
    '''
...
def check_sanity(data):
    ...
    assert all(map(lambda t: t > -273, temperatures)), 'Invalid temperatures, should be larger than -273C'
...
def output(data):
    ...
    print 'which is:       {:>9.0f} times bigger'.format(data['final_weight'] / data['initial_weight'])
